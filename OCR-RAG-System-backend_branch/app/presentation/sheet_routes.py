...
async def oauth_callback(code: str = None, state: str = None, error: str = None):
    ...
    if error:
        ...
        return RedirectResponse(url=f"/static/upload_docs.html?sheets_error=true&error={error}")

    # Handle case where user cancelled (no code and no explicit error)
    if not code:
        print("⚠️ OAuth cancelled: No authorization code received")
        return RedirectResponse(url="/static/upload_docs.html?sheets_cancelled=true")
    ...
    try:
        ...
        if not state or ":" not in state:
            ...
            return RedirectResponse(url="/static/sheets.html?sheets_error=true&error=invalid_state")
        ...
        return RedirectResponse(
            url=f"/static/upload_docs.html?sheets_connected=true&sheet_name={spreadsheet_info['spreadsheet_name']}"
        )

    except Exception as e:
        print(f"❌ OAuth callback error: {str(e)}")
        return RedirectResponse(url=f"/static/sheets.html?sheets_error=true&error={str(e)}")
