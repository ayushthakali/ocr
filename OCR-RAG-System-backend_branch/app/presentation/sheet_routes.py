...
async def get_sheets_status(current_user: dict = Depends(get_current_user)):
    ...
    try:
        # Fetch history
        # We need the google_sub, which might be in tokens if we decide to store it,
        # OR we just fetch all history for this company/user for now.
        # Ideally we stored 'sub' in tokens. Let's assume we do or will.
        # For now, let's fetch all history for the user+company scope.
        history = await firebase_service.get_sheet_history_async(user_id, company_id)
...
async def connect_sheets(current_user: dict = Depends(get_current_user)):
    """Initiate Google OAuth flow for user's active company"""
    # Generate state with user_id AND company_id to identify user + company in callback
    # Generate state with user_id AND company_id AND company_name to identify user + company in callback
    user_id = current_user["userId"]
    company_id = current_user["activeCompany"]
    company_name = current_user["companyName"]

    # Encode company name to base64 to ensure it's safe for URL/State
...
async def disconnect_sheets(current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Failed to diconnect google sheets "
        )
...
async def create_new_sheet(current_user: dict = Depends(get_current_user)):
        ...
        if expiry_str:
            ...
            if expiry_date < datetime.utcnow() + timedelta(minutes=5):
                ...
                if not refresh_token_enc:
                    raise HTTPException(401, "No refresh token available. Reconnect Google Sheets.")

                refresh_token = oauth_service.decrypt_token(refresh_token_enc)
                new_access_token, new_expiry = oauth_service.refresh_access_token(refresh_token)

                # Update tokens dict with new values
                tokens["access_token"] = oauth_service.encrypt_token(new_access_token)
                tokens["token_expiry"] = new_expiry.isoformat()

                # Save updated tokens to Firebase immediately
                await firebase_service.save_google_tokens_async(user_id, company_id, tokens)
                print("✅ Token refreshed and saved.")

    except Exception as e:
        print(f"❌ Token refresh failed: {str(e)}")
        # If refresh fails, we might still try with old token or just fail
        # Let's fail because 401 likely means old token is bad too if refresh failed
        raise HTTPException(401, f"Session expired. Please reconnect Google Sheets. Error: {str(e)}")

    try:
        # Decrypt access token (might be the new one)
        access_token = oauth_service.decrypt_token(tokens["access_token"])
        user_info = oauth_service.get_user_info(access_token)
        google_sub = user_info["id"]
    except Exception as e:
        print(f"❌ Google authentication failed: {str(e)}")
        raise HTTPException(401, "Google authentication failed. Please reconnect.")

    try:
        company_name = current_user.get("companyName", "My Company")
        sheet_number = await firebase_service.get_next_sheet_number_async(user_id, company_id)
        sheet_name = f"AI Receipt - {company_name} {sheet_number}"

        new_sheet = oauth_service.create_spreadsheet(access_token, sheet_name)
        new_sheet["created_at"] = datetime.utcnow().isoformat()
    except Exception as e:
        print(f"❌ Failed to create Google Sheet: {str(e)}")
        raise HTTPException(502, f"Failed to create Google Sheet: {str(e)}")
    ...
    try:
        ...
        print(f"❌ Failed to persist sheet data: {str(e)}")
        raise HTTPException(500, "Failed to persist sheet data")
...
async def oauth_callback(code: str = None, state: str = None, error: str = None):
    ...
    try:
        ...
        return RedirectResponse(
            url=f"{SHEETS_PAGE_URL}?sheets_error=true&error=oauth_exchange_failed"
        )

    # 2️ Validate and parse state (NO try-except)
    if not state or ":" not in state:
        return RedirectResponse(
            url=f"{SHEETS_PAGE_URL}?sheets_error=true&error=invalid_state"
        )

    parts = state.split(":")
    if len(parts) < 3:
        # Fallback for old state format or missing name
        user_id, company_id = parts[0], parts[1]
    ...
    try:
        ...
        return RedirectResponse(
            url=f"{SHEETS_PAGE_URL}?sheets_error=true&error=google_identity_failed"
        )
    ...
    try:
        ...
        return RedirectResponse(
            url=f"{SHEETS_PAGE_URL}?sheets_error=true&error=history_fetch_failed"
        )
    ...
    if not target_sheet:
        try:
            # timestamp = int(datetime.utcnow().timestamp())
            # spreadsheet_name = f"AI Receipt -({timestamp})"
            ...
            return RedirectResponse(
                url=f"{SHEETS_PAGE_URL}?sheets_error=true&error=sheet_creation_failed"
            )
        ...
        try:
            ...
            return RedirectResponse(
                url=f"{SHEETS_PAGE_URL}?sheets_error=true&error=history_save_failed"
            )
    ...
    try:
        ...
        return RedirectResponse(
            url=f"{SHEETS_PAGE_URL}?sheets_error=true&error=token_persistence_failed"
        )
